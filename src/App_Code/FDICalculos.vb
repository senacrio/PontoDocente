...
Public Class FDICalculos
    ...
    Shared Function retornaTotal(ByVal id_iniciativa As Integer, ByVal id_empresa As Integer) As Integer
        Dim i As Integer = 0
        Dim conn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("SistemaSConnectionString").ToString)
        Dim cmd As SqlCommand = New SqlCommand("SELECT (case when nMesesEntrada > nmesesSaida then nMesesEntrada else nMesesSaida end ) as total FROM dbo.fdiIniciativa where id_iniciativa=" + id_iniciativa.ToString + " and id_empresa=" + id_empresa.ToString, conn)
        ...
        Return i
    End Function
    Shared Function retornaTotalSaida(ByVal id_iniciativa As Integer, ByVal id_empresa As Integer) As Integer
        Dim i As Integer = 0
        Dim conn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("SistemaSConnectionString").ToString)
        Dim cmd As SqlCommand = New SqlCommand("SELECT nmesesSaida FROM dbo.fdiIniciativa where id_iniciativa=" + id_iniciativa.ToString + " and id_empresa=" + id_empresa.ToString, conn)
    ...
    Shared Function retornaTotalEntrada(ByVal id_iniciativa As Integer, ByVal id_empresa As Integer) As Integer
        Dim i As Integer = 0
        Dim conn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("SistemaSConnectionString").ToString)
        Dim cmd As SqlCommand = New SqlCommand("SELECT nmesesEntrada FROM dbo.fdiIniciativa where id_iniciativa=" + id_iniciativa.ToString + " and id_empresa=" + id_empresa.ToString, conn)
    ...
    Shared Function retornaValor(ByVal id_array As Integer, ByVal id_iniciativa As Integer, ByVal id_empresa As Integer) As Double
        Dim i As Double = 0


        Dim conn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("SistemaSConnectionString").ToString)
        Dim cmd As SqlCommand = New SqlCommand("SELECT valor FROM fnValorDespesa(" + id_iniciativa.ToString + "," + id_empresa.ToString + ") WHERE mes=" + id_array.ToString, conn)
    ...
    Shared Function retornaValorEntSaida(ByVal id_array As Integer, ByVal id_iniciativa As Integer, ByVal id_empresa As Integer, ByVal flag As Integer) As Double
        Dim i As Double = 0


        Dim conn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("SistemaSConnectionString").ToString)
        Dim cmd As SqlCommand = New SqlCommand("select valordespesareceita from dbo.fdiReceitasDespesas where id_iniciativa=" + id_iniciativa.ToString + " and id_empresa=" + id_empresa.ToString + " and mes=" + id_array.ToString + " and flagdespesareceita=" + flag.ToString, conn)
    ...
    Shared Function Finaliza(ByVal id_iniciativa As Integer, ByVal id_empresa As Integer) As Boolean
        Dim Saida, Entrada As Double
        Dim b As Boolean
        Dim totalSaida As Integer = retornaTotalSaida(id_iniciativa, id_empresa)
        Dim totalEntrada As Integer = retornaTotalEntrada(id_iniciativa, id_empresa)


        Saida = retornaValorEntSaida(totalSaida, id_iniciativa, id_empresa, 1)
        Entrada = retornaValorEntSaida(totalEntrada, id_iniciativa, id_empresa, 0)
    ...
    Public Sub TestIRR(ByVal TaxaRef As Decimal, ByVal vlInvestimento As Decimal, ByVal id_iniciativa As Integer, ByVal id_empresa As Integer)
        Dim Guess, RetRate, Values(retornaTotalSaida(id_iniciativa, id_empresa)), valorPayBack(retornaTotal(id_iniciativa, id_empresa)) As Double

        Dim y As Integer = 0

        Dim payback As Integer = 0 - vlInvestimento
        ...
        For x As Integer = 1 To retornaTotalSaida(id_iniciativa, id_empresa)
            Values(x) = Double.Parse(retornaValor(x, id_iniciativa, id_empresa))
        Next

        For x As Integer = 1 To retornaTotal(id_iniciativa, id_empresa)

            payback += Double.Parse(retornaValor(x, id_iniciativa, id_empresa))
            y += 1
        ...
        'Msg = "ROI = "
        'Msg = Msg & Format(RetRate, CStr(Fmt)) & " %"
        ...
        'cálculo da vpl 
        For i = 0 To retornaTotalSaida(id_iniciativa, id_empresa)
    ...
    Shared Function Finaliza() As Integer
