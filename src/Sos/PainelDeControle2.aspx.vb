...
#Region " Funções "
    ...
    Private Function TempoMedio() As String
        ...
        Dim cmd As SqlCommand = New SqlCommand("SELECT sum(DATEDIFF(n,datahoraabertura,getdate()))/count(*) as minutos from soschamado where idstatus = 1", conn)
        conn.Open()
    ...
    Private Function RetornaEmail(ByVal Matricula As String) As String
        ...
        Dim cmd As SqlCommand = New SqlCommand("select login + '@rj.senac.br' as login from admPortalUsuario where matricula=@Matricula and ativo = 1", conex)
        Dim email As String = ""
    ...
    Private Function NumSos(ByVal IDStatus As Integer) As String
        Dim cmd As SqlCommand = New SqlCommand("select count(*) as num from sosChamado where IDStatus=@IDStatus", conn)
    ...
    Private Function UltimoACP() As Integer
        Dim i As Integer
        Dim cmd As SqlCommand = New SqlCommand("select top 1 IDAcompanhamento from sosAcompanhamento where IDRegistrador=@IDRegistrador order by IDAcompanhamento desc", conn)

        Dim PIDRegistrador As SqlParameter = New SqlParameter()
        PIDRegistrador.ParameterName = "IDRegistrador"
        PIDRegistrador.Value = Session("c_Matricula")

        cmd.Parameters.Add(PIDRegistrador)
...
#End Region
    ...
    Protected Sub ImageButton4_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton4.Click
        ...
        If IsDBNull(Session("IDTecnico")) Then
            ...
            'Comunicacao.EnviaEmail("admin.comunica@rj.senac.br", email, "Sistema SOS", "Sua sos código " + lblCodigo.Text + ", com a descrição: <br /><br />'" + descricao + "', está em andamento.")
        End If
        ...
        If cboFinalizar.SelectedValue = "cancel" Then
            ...
            Comunicacao.EnviaEmail("admin.comunica@rj.senac.br", email, "Sistema SOS", "Sua sos de número: " + lblCodigo.Text + ", com a descrição: <br /><br />'" + descricao + "', foi cancelada.")
            cboFinalizar.SelectedIndex = 0
            Session("IDStatus") = 8
            grdSOS.DataBind()
            MultiView2.ActiveViewIndex = 0
        ...
        If ddlTecnicos.SelectedValue.Length <> 0 Then
            ...
            Comunicacao.EnviaEmail("admin.comunica@rj.senac.br", RetornaEmail(ddlTecnicos.SelectedValue), "Sistema SOS", "SOS código: " + lblCodigo.Text + ", com a descrição: <br /><br />'" + descricao + "', foi transferida para você.")
        ...
        Comunicacao.EnviaEmail("admin.comunica@rj.senac.br", email, "Acompanhamento SOS", "Código da SOS: " + lblCodigo.Text + ", com a descrição: <br /><br />'" + descricao + "'<br /><br />Acompanhamento: " + txtTexto.Text + "<br/> <br /> Responda esse acompanhamento via sistema. <br /> <br /> Registrado por: " + retornaNome(Session("c_Matricula")))
        ...
        'Anexa arquivos ao acompanhamento
        If grdArquivosAcp.Rows.Count <> 0 Then

            For i As Integer = 0 To grdArquivosAcp.Rows.Count - 1
                AnexoSOSACP(grdArquivosAcp.DataKeys(i).Values("IDArquivo"), UltimoACP())
            Next

        End If
        ddlTecnicos.DataBind()
        grdArquivosAcp.DataBind()
    ...
    Protected Sub btnRejeitar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRejeitar.Click
        ...
        Comunicacao.EnviaEmail("admin.comunica@rj.senac.br", RetornaEmail(RetornaIDTecnico(lblCodigoE.Text)), "SOS rejeitada.", "Sua SOS código: " + lblCodigoE.Text + " foi rejeitada." + Chr(13) + "Motivo: " + txtJustificativa.Text)
        MultiView2.ActiveViewIndex = 0
    End Sub
